...
(function (root) {
  ...
  // Item name truncation limit for the "with rate" layout — the item-name
  // column is narrower (absw4109 vs 4815) to make room for Rate + Value.
  var ITEM_LIMIT_RATE = 23250;

  function pageBodyRate(custCode, custName, orderDate, items, hdr) {
    var p = RECT + RECT;
    p += para(345,1275,1770,-240,'Customer Code       :',true);
    p += para(345,1619,1770,-240,'Customer Name      :',true);
    p += para(345,1934,1770,-240,'Address                     :',true);
    ...
    p += '{\\do\\dobxpage\\dobypage\\dprect\n\\dpx269\\dpy3528\\dpxsize11850\\dpysize330\n' +
         '\\dplinew0 \\dplinesolid\\dplinecor0\\dplinecog0\\dplinecob0\n}\n';
    p += para(449,3573,660,-240,'Sl..No',true);
    p += para(1803,3573,1770,-240,'Item Code',true);
    p += para(4081,3573,960,-240,'Item Name',true);
    p += para(7341,3573,585,-240,'UOM',true);
    p += para(8498,3573,375,-240,'Qty',true);
    p += para(9658,3573,375,-240,'Rate',true);
    p += para(10872,3573,825,-240,'Value',true);
    var i;
    for (i = 0; i < items.length; i++) p += para(345,  SL_Y[i], 720,-239, String(i+1));
    for (i = 0; i < items.length; i++) p += para(1081, ROW_Y[i],2113,-239, esc(items[i].code));
    for (i = 0; i < items.length; i++) p += para(3197, ROW_Y[i],4109,-239, esc(truncate(items[i].name, ITEM_LIMIT_RATE)));
    for (i = 0; i < items.length; i++) p += para(7371, ROW_Y[i], 383,-240, 'PCS');
    for (i = 0; i < items.length; i++) p += para(7826, ROW_Y[i],1185,-239, String(items[i].qty), false, 'qr');
    for (i = 0; i < items.length; i++) p += para(9044, ROW_Y[i],1185,-239, items[i].rate.toFixed(2), false, 'qr');
    for (i = 0; i < items.length; i++) p += para(10300,ROW_Y[i],1384,-239, items[i].value.toFixed(2), false, 'qr');
    p += '{\\do\\dobxpage\\dobypage\\dprect\n\\dpx269\\dpy13245\\dpxsize11850\\dpysize2445\n' +
         '\\dplinew0 \\dplinesolid\\dplinecor0\\dplinecog0\\dplinecob0\n}\n';
    p += para(1244,15042,1380,-240,'PREPARED BY',true);
    p += para(9522,15072,1667,-240,'APPROVED BY',true);
  ...
  var REQUIRED_RATE = ['DATE','CUST CODE','Dealer Name','PRODUCT CODE','MODEL','QTY','RATE'];
  ...
  // Same grouping as buildGroups, plus per-item rate/value tracking.
  // value is accumulated as sum(qty_row * round(rate_row,2)) so it stays
  // exact even if a product code appears on more than one row within the
  // same (date, customer) group; the displayed rate is value/qty, which
  // matches the single-row case exactly (qty * round(rate,2)).
  function buildGroupsRate(rows) {
    var map = {};
    rows.forEach(function (r) {
      var dt = normDate(r['DATE']);
      var cc = String(r['CUST CODE'] == null ? '' : r['CUST CODE']).trim();
      ...
      var pc = String(r['PRODUCT CODE'] || '').trim();
      if (!pc) return;
      if (!map[key].items[pc]) map[key].items[pc] = { code: pc, name: String(r['MODEL'] || ''), qty: 0, value: 0 };
      var q = Number(r['QTY']) || 0;
      var rt = Math.round((Number(r['RATE']) || 0) * 100) / 100;
      map[key].items[pc].qty += q;
      map[key].items[pc].value += q * rt;
    });
    var groups = Object.keys(map).map(function (k) {
      var g = map[k];
      var items = Object.keys(g.items).sort().map(function (pc) {
        var it = g.items[pc];
        it.qty = Math.round(it.qty);
        it.value = Math.round(it.value * 100) / 100;
        it.rate = it.qty ? Math.round((it.value / it.qty) * 100) / 100 : 0;
        return it;
      });
      return { date: g.date, cust: g.cust, name: g.name, items: items };
    });
  ...
  function generateDocRate(rows, title, header) {
    var hdr = Object.assign({}, DEFAULT_HEADER, header || {});
    var groups = buildGroupsRate(rows);
    if (!groups.length) throw new Error('No valid data rows (DATE / CUST CODE missing).');
    var now = new Date();
    var doc = '{\\rtf\\ansi\n{\\fonttbl{\\f1 Times New Roman}}\n{\\colortbl;\\red0\\green0\\blue0;}\n' +
      '{\\info {\\creatim \\yr' + now.getFullYear() + ' \\mo\\' + MONTHS[now.getMonth()] +
      ...
      esc(title) + '}  }\n\\viewkind1\n\\paperw16560\\paperh16560\n\\lndscpsxn\n';
    groups.forEach(function (g, idx) {
      var body = pageBodyRate(g.cust, g.name, g.date, g.items, hdr);
      if (idx === 0) {
        doc += '{{\\pard \\phpg\\pvpg\\posx0\\posy0\\absw5000\\absh-1\\nowrap      ' +
               '{This file was created by Oracle Reports. Please view this document in Page Layout mode.}\\par}\n' +
               body + '}\n';
    ...
    var totalValue = groups.reduce(function (s, g) {
  ...
  var api = {
    ...
    buildGroupsRate: buildGroupsRate,
    generateDoc: generateDoc,
    generateDocRate: generateDocRate,
    formatDate: fdate,
    normDate: normDate
  };
